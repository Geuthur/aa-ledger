...
class CharManagerQuerySetTest(TestCase):
    ...
    def test_annotate_bounty(self):
        ...
        qs = self.manager.annotate_bounty(character_ids, filter_date)
    ...
    def test_annotate_ess(self):
        ...
        qs = self.manager.annotate_ess(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_ess", qs.query.annotations)

        qs_no_filter = self.manager.annotate_ess(character_ids)
        self.assertIsNotNone(qs_no_filter)
    ...
    def test_annotate_mission(self):
        ...
        qs = self.manager.annotate_mission(character_ids, filter_date)
        self.assertIsNotNone(qs)
    ...
    def test_annotate_contract_cost(self):
        ...
        qs = self.manager.annotate_contract_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
    ...
    def test_annotate_market_cost(self, mock_get_models_and_string):
        ...
        qs = manager.annotate_market_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_market_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_market_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_market_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_assets_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_assets_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_assets_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_assets_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_assets_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_traveling_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_traveling_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_traveling_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_traveling_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_traveling_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_production_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_production_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_production_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_production_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_production_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_skill_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_skill_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_skill_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_skill_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_skill_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_insurance_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_insurance_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_insurance_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_insurance_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_insurance_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_planetary_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_planetary_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_planetary_cost", qs.query.annotations)

        qs_no_filter = manager.annotate_planetary_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_planetary_cost", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_lp_cost(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_lp_cost(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_lp", qs.query.annotations)

        qs_no_filter = manager.annotate_lp_cost(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_lp", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_market_trade(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_market_trade(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_market_trade", qs.query.annotations)

        qs_no_filter = manager.annotate_market_trade(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_market_trade", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_contract_trade(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_contract_trade(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_contract_trade", qs.query.annotations)

        qs_no_filter = manager.annotate_contract_trade(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_contract_trade", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_donation_trade(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_donation_trade(character_ids, filter_date, exclude)
        self.assertIsNotNone(qs)
        self.assertIn("total_donation_trade", qs.query.annotations)

        qs_no_filter = manager.annotate_donation_trade(character_ids)
        self.assertIsNotNone(qs_no_filter)
        self.assertIn("total_donation_trade", qs_no_filter.query.annotations)

    @patch("ledger.hooks.get_models_and_string")
    def test_annotate_insurance_trade(self, mock_get_models_and_string):
        mock_model = MagicMock()
        mock_get_models_and_string.return_value = (None, mock_model)
        manager = CharWalletManager()
        ...
        qs = manager.annotate_insurance_trade(character_ids, filter_date)
        self.assertIsNotNone(qs)
        self.assertIn("total_insurance_trade", qs.query.annotations)

        qs_no_filter = manager.annotate_insurance_trade(character_ids)
        self.assertIsNotNone(qs_no_filter)
    ...
    def test_annotate_ledger(self, mock_get_models_and_string):
        ...
        # Test with filter
        result_with_filter = manager.annotate_ledger(
        ...
        # Test without filter
        result_without_filter = manager.annotate_ledger(character_ids)
